...
var AstNode = function AstNode(line, column) {
  this.line = line;
  this.column = column;
};

AstNode.prototype._sn = function (indent, fileName, chunk) {
  return new SourceNode(this.line, this.column, fileName, '').add(indentNode(indent)).add(chunk);
};
...
PrintExpression.prototype.compile = function (indent, fileName) {
  return this._sn(indent, fileName, 'console.log( ').add(this.value.compile(indent, fileName)).add(' );\n');
};
...
IntDeclarationExpression.prototype.compile = function (indent, fileName) {
  ...
  node.add('var ').add(this.name.compile(indent, fileName)).add(' = ').add(val);
  return node.add(';\n');
};
...
AssignementExpression.prototype.compile = function (indent, fileName) {
  ...
  var node = this._sn(indent, fileName, 'var ').add(compiledName).add(' = (');
  ...
  if (this.operations && this.operations.length > 0) {
    ...
    node.add(opsNodes).add(');\n');
  } else {
    node.add(this.initialValue.compile(indent, fileName)).add(');\n');
  }
  node.add(indentNode(indent)).add('if(typeof(').add(compiledName).add(') === "boolean") { ').add(compiledName).add(' = ').add(compiledName).add(' ? 1 : 0; }\n').add(indentNode(indent)).add(compiledName).add(' = Math.round(').add(compiledName).add(');\n');
...
IfExpression.prototype.compile = function (indent, fileName) {
  var expr = this._sn(indent, fileName, 'if (').add(this.predicate.compile(indent, fileName)).add(') { \n');
  ...
  if (this.elseStatements && this.elseStatements.length > 0) {
    expr.add(indentNode(indent)).add('}\n').add(this.elseNode.compile(indent, fileName)).add(this.elseStatements.map(function (node) {
      return node.compile(indent + 1, fileName);
    }));
  }
  return expr.add(indentNode(indent)).add(new SourceNode(this.endIfLine, this.endIfColumn, fileName, '}\n'));
};
...
WhileExpression.prototype.compile = function (indent, fileName) {
  return this._sn(indent, fileName, 'while (').add(this.predicate.compile(indent, fileName)).add(') {\n').add(this.whileStatements.map(function (statement) {
    return statement.compile(indent + 1, fileName);
  })).add(indentNode(indent)).add(new SourceNode(this.endLine, this.endColumn, fileName, '}\n'));
};
...
MethodDeclarationExpression.prototype.compile = function (indent, fileName) {
  var node = this._sn(indent, fileName, 'function ').add(this.name.compile(indent, fileName)).add(' (');
...
CallExpression.prototype.compile = function (indent, fileName) {
  var node = this._sn(indent, fileName, '').add(this.name.compile(indent, fileName)).add('(');
...
ReturnExpression.prototype.compile = function (indent, fileName) {
  return this._sn(indent, fileName, 'return ').add(this.value.compile(indent, fileName)).add(';\n');
};
...
Operation.prototype.compile = function (indent, fileName) {
  return this._sn(0, fileName, '').add(this.operation).add(this.variable.compile(indent, fileName));
};
...
AssignementFromCallExpression.prototype.compile = function (indent, fileName) {
  return this._sn(indent, fileName, 'var ').add(this.name.compile(indent, fileName)).add(' = ').add(this.functionCalled.compile(0, fileName));
};
...
ArgumentDeclarationExpression.prototype.compile = function (indent, fileName) {
  return this._sn(indent, fileName, '').add(this.variable.compile(indent, fileName));
};
